...
TreeNode* findPred(TreeNode* root, int val) {
	if(root == NULL) return NULL;
	TreeNode* currentNode = search(root, val);
...
int main(){	
	...
	TreeNode* predNode = findPred(root, 70);
	TreeNode* succNode = findSucc(root, 70);

	std::cout << predNode->val << std::endl;
	std::cout << succNode->val << std::endl;
